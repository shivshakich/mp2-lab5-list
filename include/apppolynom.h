...
bool SetPol(AppPolynom&, string&);
bool SetLeft(AppPolynom&, string&);
bool SetOp(AppPolynom&, string&);
bool SetRight(AppPolynom&, string&);
bool Perform(AppPolynom&);

int GetSize(AppPolynom& app) { return app.vec.size(); }
...
bool SetPol(AppPolynom& app, string& in)
{
	try { TPolynom tp(in); }
	catch (...) { return false; }
...
bool SetLeft(AppPolynom& app, string& in)
{
	bool res = true;
...
// допустимые символы: + - * / =
bool SetOp(AppPolynom& app, string& in)
{
	bool res = false;
	char c = in[0];
...
bool SetRight(AppPolynom& app, string& in)
{
	bool res = true;
